...
class Monster:
    ...
    def __init__(self, name, difficulty_multiplier):
        ...
        # Reduced multiplier for regular monsters
        if name not in ["Troll", "Dragon"]:
            difficulty_multiplier = 1 + (difficulty_multiplier * 0.05)  # Much smaller scaling
    ...
    def battle(self, is_boss=False):
        ...
        if is_boss:
            if self.current_layer == self.layers - 1:
                enemy = Monster("Dragon", difficulty_multiplier * 1.2)  # Dragon is extra tough
            else:
                enemy = Monster("Troll", difficulty_multiplier)
        else:
            monster_types = ["Goblin", "Skeleton", "Orc"]  # Remove Troll from regular battles
