...
class Character:
    ...
    def use_potion(self, potion):
        if potion.type == "Health":
            self.hp = min(self.max_hp, self.hp + potion.effect["heal"])
        elif potion.type == "Mana":
            self.mana = min(self.max_mana, self.mana + potion.effect["restore"])
...
class Monster:
    MONSTER_TYPES = {
        ...
        "Dragon": {"hp": 50, "attack": 10, "defense": 8, "xp_reward": 50, "drop_chance": 0.8}
    }
...
class Maze:
        ...
        for y in range(self.height):
            ...
            for x in range(self.width):
                if (x, y) == self.player_pos:
                    ...
                    if (x, y) in self.visited_rooms:
                        row += f" . "  # Always show as explored
                    else:
                        row += " ? "
                else:
                    row += "   "
    ...
    def move(self, direction):
        ...
        if (0 <= new_pos[0] < self.width and 0 <= new_pos[1] < self.height and 
            ...
            self.enter_room(self.rooms[new_pos])
            self.print_maze()
    ...
    def enter_room(self, room_type):
        ...
        if room_type == "battle":
            ...
            self.player.mana = min(self.player.mana + 5, self.player.max_mana)
    ...
    def battle(self, is_boss=False):
        # Calculate difficulty multiplier based on current layer
        difficulty_multiplier = 1 + (self.current_layer * 0.2)
        
        monster_types = list(Monster.MONSTER_TYPES.keys())
        if is_boss:
        ...
        potion_used = False  # Track if potion was used this turn
        ...
        while self.player.hp > 0 and enemy.hp > 0:
            ...
            if action == "p":
                ...
                try:
                    ...
                    if 0 <= potion_idx < len(potions):
                        potion = potions.pop(potion_idx)
                        self.player.use_potion(potion)
                        print(f"Used {potion.name}")
                        potion_used = True  # Mark potion used to allow enemy turn
                    else:
            ...
            if action == "a":
                # Player attack
                damage = max(0, self.player.attack - enemy.defense + random.randint(-2, 2))
                enemy.hp -= damage
                ...
                try:
                    ...
                    skill = self.player.use_skill(skill_level)
                    if skill:
                        if "damage" in skill:
                            enemy.hp -= skill["damage"]
                            print(f"You used {skill['name']} and dealt {skill['damage']} damage!")
                        elif "healing" in skill:
                            self.player.hp = min(self.player.max_hp, self.player.hp + skill["healing"])
                ...
                item_choice = input("Select an item to equip (number) or 'c' to cancel: ")
                if item_choice.lower() == 'c':
                    continue
                try:
                    item_idx = int(item_choice) - 1
                    if 0 <= item_idx < len(self.player.inventory):
                        item = self.player.inventory.pop(item_idx)
                        replaced_item = self.player.equip(item)
                        if replaced_item:
                            self.player.add_to_inventory(replaced_item)
            ...
            # Enemy turn - happens after player action, including when potion is used
            if enemy.hp > 0 and (action != "p" or potion_used):
                # Enemy attack
                damage = max(0, enemy.attack - self.player.defense + random.randint(-1, 1))
                self.player.hp -= damage
                print(f"{enemy.name} dealt {damage} damage to you!")
                potion_used = False  # Reset potion flag
        ...
        if enemy.hp <= 0:
            ...
            # Still keep the existing potion drop chance
